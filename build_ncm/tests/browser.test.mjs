...
try {
  ...
  assert.match(initial.totalBuildingCount, /81 BUILDINGS/);
  ...
  await clickBuilding(client, "hollow-cottage");
  ...
  assert.ok(!cottageLoaded.resources.includes("/build_ncm/buildings/residential/compact-village-farmhouse.json"), "selecting the hollow cottage must not load the farmhouse JSON");
  assert.ok(!cottageLoaded.resources.includes("/build_ncm/concepts/residential/compact-village-farmhouse.webp"), "selecting the hollow cottage must not load the farmhouse concept art");
  assert.ok(!cottageLoaded.resources.includes("/build_ncm/buildings/residential/compact-village-rowhouse.json"), "selecting the hollow cottage must not load the rowhouse JSON");
  assert.ok(!cottageLoaded.resources.includes("/build_ncm/concepts/residential/compact-village-rowhouse.webp"), "selecting the hollow cottage must not load the rowhouse concept art");
  assert.ok(!cottageLoaded.resources.includes("/build_ncm/buildings/residential/compact-village-boarding-house.json"), "selecting the hollow cottage must not load the boarding-house JSON");
  assert.ok(!cottageLoaded.resources.includes("/build_ncm/concepts/residential/compact-village-boarding-house.webp"), "selecting the hollow cottage must not load the boarding-house concept art");
  ...
  await clickBuilding(client, "compact-village-duplex");
  ...
  await clickBuilding(client, "compact-village-farmhouse");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-farmhouse' && document.querySelector('#modelSize').textContent === '23 × 18 × 27' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const farmhouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(farmhouse.activeCategory, "residential");
  assert.match(farmhouse.title, /Compact Village Farmhouse/);
  assert.equal(farmhouse.modelSize, "23 × 18 × 27");
  assert.match(farmhouse.payload, /^NCM3:/);
  assert.equal(farmhouse.voxelCount, 2361);
  for (const id of [55, 56, 57, 58, 62, 68, 69, 70, 96]) assert.match(farmhouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(farmhouse.uncovered, false);
  assert.equal(farmhouse.glazingDisabled, true);
  assert.equal(farmhouse.glazingLabel, "Openings: Not applicable");
  assert.equal(farmhouse.disabledStyles, 6);
  assert.equal(farmhouse.disabledRoofs, 6);
  assert.equal(farmhouse.conceptHidden, false);
  assert.equal(farmhouse.conceptLoading, "eager");
  assert.match(farmhouse.conceptAlt, /Compact Village Farmhouse concept reference/);
  assert.equal(farmhouse.conceptFit, "contain");
  assert.equal(farmhouse.selectedInUrl, "compact-village-farmhouse");
  assert.ok(farmhouse.resources.includes("/build_ncm/buildings/residential/compact-village-farmhouse.json"));
  assert.ok(farmhouse.resources.includes("/build_ncm/concepts/residential/compact-village-farmhouse.webp"));
  assert.ok(!farmhouse.resources.some((path) => path.endsWith("compact-village-farmhouse-blueprint.js")));
  assert.ok(!farmhouse.resources.includes("/build_ncm/buildings/residential/compact-village-rowhouse.json"), "selecting existing residential buildings must not load the rowhouse JSON");
  assert.ok(!farmhouse.resources.includes("/build_ncm/concepts/residential/compact-village-rowhouse.webp"), "selecting existing residential buildings must not load the rowhouse concept art");

  await clickBuilding(client, "compact-village-rowhouse");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-rowhouse' && document.querySelector('#modelSize').textContent === '37 × 21 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const rowhouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(rowhouse.activeCategory, "residential");
  assert.match(rowhouse.title, /Compact Village Rowhouse/);
  assert.equal(rowhouse.modelSize, "37 × 21 × 25");
  assert.match(rowhouse.payload, /^NCM3:/);
  assert.equal(rowhouse.voxelCount, 2786);
  for (const id of [55, 57, 58, 62, 68, 69, 70, 96]) assert.match(rowhouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(rowhouse.uncovered, false);
  assert.equal(rowhouse.glazingDisabled, true);
  assert.equal(rowhouse.glazingLabel, "Openings: Not applicable");
  assert.equal(rowhouse.disabledStyles, 6);
  assert.equal(rowhouse.disabledRoofs, 6);
  assert.equal(rowhouse.conceptHidden, false);
  assert.equal(rowhouse.conceptLoading, "eager");
  assert.match(rowhouse.conceptAlt, /Compact Village Rowhouse concept reference/);
  assert.equal(rowhouse.conceptFit, "contain");
  assert.equal(rowhouse.selectedInUrl, "compact-village-rowhouse");
  assert.ok(rowhouse.resources.includes("/build_ncm/buildings/residential/compact-village-rowhouse.json"));
  assert.ok(rowhouse.resources.includes("/build_ncm/concepts/residential/compact-village-rowhouse.webp"));
  assert.ok(!rowhouse.resources.some((path) => path.endsWith("compact-village-rowhouse-blueprint.js")));

  await clickBuilding(client, "compact-village-boarding-house");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-boarding-house' && document.querySelector('#modelSize').textContent === '29 × 19 × 35' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const boardingHouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(boardingHouse.activeCategory, "residential");
  assert.match(boardingHouse.title, /Compact Village Boarding House/);
  assert.equal(boardingHouse.modelSize, "29 × 19 × 35");
  assert.match(boardingHouse.payload, /^NCM3:/);
  assert.equal(boardingHouse.voxelCount, 3654);
  for (const id of [55, 56, 57, 58, 62, 68, 69, 70, 96]) assert.match(boardingHouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(boardingHouse.uncovered, false);
  assert.equal(boardingHouse.glazingDisabled, true);
  assert.equal(boardingHouse.glazingLabel, "Openings: Not applicable");
  assert.equal(boardingHouse.disabledStyles, 6);
  assert.equal(boardingHouse.disabledRoofs, 6);
  assert.equal(boardingHouse.conceptHidden, false);
  assert.equal(boardingHouse.conceptLoading, "eager");
  assert.match(boardingHouse.conceptAlt, /Compact Village Boarding House concept reference/);
  assert.equal(boardingHouse.conceptFit, "contain");
  assert.equal(boardingHouse.selectedInUrl, "compact-village-boarding-house");
  assert.ok(boardingHouse.resources.includes("/build_ncm/buildings/residential/compact-village-boarding-house.json"));
  assert.ok(boardingHouse.resources.includes("/build_ncm/concepts/residential/compact-village-boarding-house.webp"));
  assert.ok(!boardingHouse.resources.some((path) => path.endsWith("compact-village-boarding-house-blueprint.js")));

  await evaluate(client, "document.querySelector('[data-building-category=coastal]').click()");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building-category].active')?.dataset.buildingCategory === 'coastal' && document.querySelector('[data-building=seaside-cottage]') && document.querySelector('[data-building=stone-timber-harbor-beacon]') && document.querySelector('[data-building=compact-village-smokehouse]') && document.querySelector('[data-building=compact-village-fish-market]') && document.querySelector('[data-building=compact-village-boatwright-workshop]') && document.querySelector('[data-building=compact-village-boathouse]')"));
  ...
  assert.equal(coastalBrowse.cardCount, 6);
  assert.match(coastalBrowse.previewTitle, /Compact Village Boarding House/);
  ...
  assert.ok(!coastalBrowse.resources.includes("/build_ncm/buildings/coastal/compact-village-fish-market.json"), "category browsing must not load the fish-market JSON");
  assert.ok(!coastalBrowse.resources.includes("/build_ncm/concepts/coastal/compact-village-fish-market.webp"), "category browsing must not load the fish-market concept art");
  assert.ok(!coastalBrowse.resources.includes("/build_ncm/buildings/coastal/compact-village-boatwright-workshop.json"), "category browsing must not load the boatwright-workshop JSON");
  assert.ok(!coastalBrowse.resources.includes("/build_ncm/concepts/coastal/compact-village-boatwright-workshop.webp"), "category browsing must not load the boatwright-workshop concept art");
  assert.ok(!coastalBrowse.resources.includes("/build_ncm/buildings/coastal/compact-village-boathouse.json"), "category browsing must not load the boathouse JSON");
  assert.ok(!coastalBrowse.resources.includes("/build_ncm/concepts/coastal/compact-village-boathouse.webp"), "category browsing must not load the boathouse concept art");
  ...
  assert.ok(!smokehouse.resources.includes("/build_ncm/buildings/coastal/compact-village-fish-market.json"), "selecting existing coastal buildings must not load the fish-market JSON");
  assert.ok(!smokehouse.resources.includes("/build_ncm/concepts/coastal/compact-village-fish-market.webp"), "selecting existing coastal buildings must not load the fish-market concept art");

  await clickBuilding(client, "compact-village-fish-market");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-fish-market' && document.querySelector('#modelSize').textContent === '27 × 21 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const fishMarket = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(fishMarket.activeCategory, "coastal");
  assert.match(fishMarket.title, /Compact Village Fish Market/);
  assert.equal(fishMarket.modelSize, "27 × 21 × 25");
  assert.match(fishMarket.payload, /^NCM3:/);
  assert.equal(fishMarket.voxelCount, 1638);
  for (const id of [55, 57, 59, 68, 69, 70, 73, 77, 97]) assert.match(fishMarket.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(fishMarket.uncovered, false);
  assert.equal(fishMarket.glazingDisabled, true);
  assert.equal(fishMarket.glazingLabel, "Openings: Not applicable");
  assert.equal(fishMarket.disabledStyles, 6);
  assert.equal(fishMarket.disabledRoofs, 6);
  assert.equal(fishMarket.conceptHidden, false);
  assert.equal(fishMarket.conceptLoading, "eager");
  assert.match(fishMarket.conceptAlt, /Compact Village Fish Market concept reference/);
  assert.equal(fishMarket.conceptFit, "contain");
  assert.equal(fishMarket.selectedInUrl, "compact-village-fish-market");
  assert.ok(fishMarket.resources.includes("/build_ncm/buildings/coastal/compact-village-fish-market.json"));
  assert.ok(fishMarket.resources.includes("/build_ncm/concepts/coastal/compact-village-fish-market.webp"));
  assert.ok(!fishMarket.resources.some((path) => path.endsWith("compact-village-fish-market-blueprint.js")));

  await clickBuilding(client, "compact-village-boatwright-workshop");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-boatwright-workshop' && document.querySelector('#modelSize').textContent === '27 × 22 × 31' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const boatwrightWorkshop = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(boatwrightWorkshop.activeCategory, "coastal");
  assert.match(boatwrightWorkshop.title, /Compact Village Boatwright Workshop/);
  assert.equal(boatwrightWorkshop.modelSize, "27 × 22 × 31");
  assert.match(boatwrightWorkshop.payload, /^NCM3:/);
  assert.equal(boatwrightWorkshop.voxelCount, 2351);
  for (const id of [55, 56, 57, 59, 68, 69, 70, 73, 97]) assert.match(boatwrightWorkshop.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(boatwrightWorkshop.uncovered, false);
  assert.equal(boatwrightWorkshop.glazingDisabled, true);
  assert.equal(boatwrightWorkshop.glazingLabel, "Openings: Not applicable");
  assert.equal(boatwrightWorkshop.disabledStyles, 6);
  assert.equal(boatwrightWorkshop.disabledRoofs, 6);
  assert.equal(boatwrightWorkshop.conceptHidden, false);
  assert.equal(boatwrightWorkshop.conceptLoading, "eager");
  assert.match(boatwrightWorkshop.conceptAlt, /Compact Village Boatwright Workshop concept reference/);
  assert.equal(boatwrightWorkshop.conceptFit, "contain");
  assert.equal(boatwrightWorkshop.selectedInUrl, "compact-village-boatwright-workshop");
  assert.ok(boatwrightWorkshop.resources.includes("/build_ncm/buildings/coastal/compact-village-boatwright-workshop.json"));
  assert.ok(boatwrightWorkshop.resources.includes("/build_ncm/concepts/coastal/compact-village-boatwright-workshop.webp"));
  assert.ok(!boatwrightWorkshop.resources.some((path) => path.endsWith("compact-village-boatwright-workshop-blueprint.js")));
  assert.ok(!boatwrightWorkshop.resources.includes("/build_ncm/buildings/coastal/compact-village-boathouse.json"), "selecting existing coastal buildings must not load the boathouse JSON");
  assert.ok(!boatwrightWorkshop.resources.includes("/build_ncm/concepts/coastal/compact-village-boathouse.webp"), "selecting existing coastal buildings must not load the boathouse concept art");

  await clickBuilding(client, "compact-village-boathouse");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-boathouse' && document.querySelector('#modelSize').textContent === '35 × 22 × 31' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const boathouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(boathouse.activeCategory, "coastal");
  assert.match(boathouse.title, /Compact Village Boathouse/);
  assert.equal(boathouse.modelSize, "35 × 22 × 31");
  assert.match(boathouse.payload, /^NCM3:/);
  assert.equal(boathouse.voxelCount, 4860);
  for (const id of [55, 57, 59, 68, 70, 73, 97]) assert.match(boathouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(boathouse.uncovered, false);
  assert.equal(boathouse.glazingDisabled, true);
  assert.equal(boathouse.glazingLabel, "Openings: Not applicable");
  assert.equal(boathouse.disabledStyles, 6);
  assert.equal(boathouse.disabledRoofs, 6);
  assert.equal(boathouse.conceptHidden, false);
  assert.equal(boathouse.conceptLoading, "eager");
  assert.match(boathouse.conceptAlt, /Compact Village Boathouse concept reference/);
  assert.equal(boathouse.conceptFit, "contain");
  assert.equal(boathouse.selectedInUrl, "compact-village-boathouse");
  assert.ok(boathouse.resources.includes("/build_ncm/buildings/coastal/compact-village-boathouse.json"));
  assert.ok(boathouse.resources.includes("/build_ncm/concepts/coastal/compact-village-boathouse.webp"));
  assert.ok(!boathouse.resources.some((path) => path.endsWith("compact-village-boathouse-blueprint.js")));
  ...
  assert.ok(!industrialBrowse.resources.includes("/build_ncm/buildings/industrial/compact-village-brewhouse.json"), "category browsing must not load the brewhouse JSON");
  ...
  assert.ok(!warehouse.resources.includes("/build_ncm/buildings/industrial/compact-village-blacksmith-shop.json"), "selecting the warehouse must not load the blacksmith-shop JSON");
  assert.ok(!warehouse.resources.includes("/build_ncm/concepts/industrial/compact-village-blacksmith-shop.webp"), "selecting the warehouse must not load the blacksmith-shop concept art");

  await clickBuilding(client, "compact-village-blacksmith-shop");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-blacksmith-shop' && document.querySelector('#modelSize').textContent === '23 × 18 × 21' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const blacksmithShop = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(blacksmithShop.activeCategory, "industrial");
  assert.match(blacksmithShop.title, /Compact Village Blacksmith Shop/);
  assert.equal(blacksmithShop.modelSize, "23 × 18 × 21");
  assert.match(blacksmithShop.payload, /^NCM3:/);
  assert.equal(blacksmithShop.voxelCount, 2106);
  for (const id of [55, 56, 57, 58, 60, 62, 65, 66, 68, 70, 72, 96]) assert.match(blacksmithShop.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(blacksmithShop.uncovered, false);
  assert.equal(blacksmithShop.glazingDisabled, true);
  assert.equal(blacksmithShop.glazingLabel, "Openings: Not applicable");
  assert.equal(blacksmithShop.disabledStyles, 6);
  assert.equal(blacksmithShop.disabledRoofs, 6);
  assert.equal(blacksmithShop.conceptHidden, false);
  assert.match(blacksmithShop.conceptAlt, /Compact Village Blacksmith Shop concept reference/);
  assert.equal(blacksmithShop.conceptFit, "contain");
  assert.equal(blacksmithShop.selectedInUrl, "compact-village-blacksmith-shop");
  ...
  await clickBuilding(client, "compact-village-weaving-workshop");
  ...
  await clickBuilding(client, "compact-village-pottery-workshop");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-pottery-workshop' && document.querySelector('#modelSize').textContent === '23 × 19 × 23' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const potteryWorkshop = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(potteryWorkshop.activeCategory, "industrial");
  assert.match(potteryWorkshop.title, /Compact Village Pottery Workshop/);
  assert.equal(potteryWorkshop.modelSize, "23 × 19 × 23");
  assert.match(potteryWorkshop.payload, /^NCM3:/);
  assert.equal(potteryWorkshop.voxelCount, 2055);
  for (const id of [55, 56, 57, 58, 62, 68, 69, 70, 72, 75, 96]) assert.match(potteryWorkshop.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(potteryWorkshop.uncovered, false);
  assert.equal(potteryWorkshop.glazingDisabled, true);
  assert.equal(potteryWorkshop.glazingLabel, "Openings: Not applicable");
  assert.equal(potteryWorkshop.disabledStyles, 6);
  assert.equal(potteryWorkshop.disabledRoofs, 6);
  assert.equal(potteryWorkshop.conceptHidden, false);
  assert.equal(potteryWorkshop.conceptLoading, "eager");
  assert.match(potteryWorkshop.conceptAlt, /Compact Village Pottery Workshop concept reference/);
  assert.equal(potteryWorkshop.conceptFit, "contain");
  assert.equal(potteryWorkshop.selectedInUrl, "compact-village-pottery-workshop");
  ...
  assert.ok(!cooperWorkshop.resources.includes("/build_ncm/buildings/industrial/compact-village-brewhouse.json"), "selecting the cooper workshop must not load the brewhouse JSON");
  assert.ok(!cooperWorkshop.resources.includes("/build_ncm/concepts/industrial/compact-village-brewhouse.webp"), "selecting the cooper workshop must not load the brewhouse concept art");

  await clickBuilding(client, "compact-village-brewhouse");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-brewhouse' && document.querySelector('#modelSize').textContent === '25 × 25 × 27' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const brewhouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(brewhouse.activeCategory, "industrial");
  assert.match(brewhouse.title, /Compact Village Brewhouse/);
  assert.equal(brewhouse.modelSize, "25 × 25 × 27");
  assert.match(brewhouse.payload, /^NCM3:/);
  assert.equal(brewhouse.voxelCount, 2512);
  for (const id of [55, 56, 57, 58, 62, 65, 68, 69, 70, 96]) assert.match(brewhouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(brewhouse.uncovered, false);
  assert.equal(brewhouse.glazingDisabled, true);
  assert.equal(brewhouse.glazingLabel, "Openings: Not applicable");
  assert.equal(brewhouse.disabledStyles, 6);
  assert.equal(brewhouse.disabledRoofs, 6);
  assert.equal(brewhouse.conceptHidden, false);
  assert.equal(brewhouse.conceptLoading, "eager");
  assert.match(brewhouse.conceptAlt, /Compact Village Brewhouse concept reference/);
  assert.equal(brewhouse.conceptFit, "contain");
  assert.equal(brewhouse.selectedInUrl, "compact-village-brewhouse");
  assert.ok(brewhouse.resources.includes("/build_ncm/buildings/industrial/compact-village-brewhouse.json"));
  assert.ok(brewhouse.resources.includes("/build_ncm/concepts/industrial/compact-village-brewhouse.webp"));
  assert.ok(!brewhouse.resources.some((path) => path.endsWith("compact-village-brewhouse-blueprint.js")));
  assert.ok(!brewhouse.resources.includes("/build_ncm/buildings/industrial/compact-village-tannery.json"), "selecting existing industrial buildings must not load the tannery JSON");
  assert.ok(!brewhouse.resources.includes("/build_ncm/concepts/industrial/compact-village-tannery.webp"), "selecting existing industrial buildings must not load the tannery concept art");

  await clickBuilding(client, "compact-village-tannery");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-tannery' && document.querySelector('#modelSize').textContent === '27 × 21 × 27' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const tannery = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(tannery.activeCategory, "industrial");
  assert.match(tannery.title, /Compact Village Tannery/);
  assert.equal(tannery.modelSize, "27 × 21 × 27");
  assert.match(tannery.payload, /^NCM3:/);
  assert.equal(tannery.voxelCount, 1757);
  for (const id of [55, 56, 57, 58, 60, 65, 68, 69, 70, 71, 74, 96]) assert.match(tannery.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(tannery.uncovered, false);
  assert.equal(tannery.glazingDisabled, true);
  assert.equal(tannery.glazingLabel, "Openings: Not applicable");
  assert.equal(tannery.disabledStyles, 6);
  assert.equal(tannery.disabledRoofs, 6);
  assert.equal(tannery.conceptHidden, false);
  assert.equal(tannery.conceptLoading, "eager");
  assert.match(tannery.conceptAlt, /Compact Village Tannery concept reference/);
  assert.equal(tannery.conceptFit, "contain");
  assert.equal(tannery.selectedInUrl, "compact-village-tannery");
  ...
  await clickBuilding(client, "compact-village-candlemaker-workshop");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-candlemaker-workshop' && document.querySelector('#modelSize').textContent === '27 × 19 × 23' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const candlemakerWorkshop = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(candlemakerWorkshop.activeCategory, "industrial");
  assert.match(candlemakerWorkshop.title, /Compact Village Candlemaker Workshop/);
  assert.equal(candlemakerWorkshop.modelSize, "27 × 19 × 23");
  assert.match(candlemakerWorkshop.payload, /^NCM3:/);
  assert.equal(candlemakerWorkshop.voxelCount, 2270);
  for (const id of [56, 57, 60, 62, 65, 68, 69, 70, 74, 96]) assert.match(candlemakerWorkshop.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(candlemakerWorkshop.uncovered, false);
  assert.equal(candlemakerWorkshop.glazingDisabled, true);
  assert.equal(candlemakerWorkshop.glazingLabel, "Openings: Not applicable");
  assert.equal(candlemakerWorkshop.disabledStyles, 6);
  assert.equal(candlemakerWorkshop.disabledRoofs, 6);
  assert.equal(candlemakerWorkshop.conceptHidden, false);
  assert.equal(candlemakerWorkshop.conceptLoading, "eager");
  assert.match(candlemakerWorkshop.conceptAlt, /Compact Village Candlemaker Workshop concept reference/);
  assert.equal(candlemakerWorkshop.conceptFit, "contain");
  assert.equal(candlemakerWorkshop.selectedInUrl, "compact-village-candlemaker-workshop");
  ...
  const watchtower = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(watchtower.activeCategory, "fortress");
  assert.match(watchtower.title, /Compact Village Watchtower/);
  assert.equal(watchtower.modelSize, "21 × 41 × 21");
  assert.match(watchtower.payload, /^NCM3:/);
  assert.equal(watchtower.voxelCount, 3641);
  for (const id of [55, 56, 57, 58, 68, 69, 70, 96]) assert.match(watchtower.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(watchtower.uncovered, false);
  assert.equal(watchtower.glazingDisabled, true);
  assert.equal(watchtower.glazingLabel, "Openings: Not applicable");
  assert.equal(watchtower.disabledStyles, 6);
  assert.equal(watchtower.disabledRoofs, 6);
  assert.equal(watchtower.conceptHidden, false);
  assert.equal(watchtower.conceptLoading, "eager");
  assert.match(watchtower.conceptAlt, /Compact Village Watchtower concept reference/);
  assert.equal(watchtower.conceptFit, "contain");
  assert.equal(watchtower.selectedInUrl, "compact-village-watchtower");
  ...
  assert.equal(civicBrowse.cardCount, 9);
  ...
  assert.ok(!civicBrowse.resources.includes("/build_ncm/buildings/civic/compact-village-chapel.json"), "category browsing must not load the chapel JSON");
  assert.ok(!civicBrowse.resources.includes("/build_ncm/concepts/civic/compact-village-chapel.webp"), "category browsing must not load the chapel concept art");
  assert.ok(!civicBrowse.resources.includes("/build_ncm/buildings/civic/compact-village-bathhouse.json"), "category browsing must not load the bathhouse JSON");
  assert.ok(!civicBrowse.resources.includes("/build_ncm/concepts/civic/compact-village-bathhouse.webp"), "category browsing must not load the bathhouse concept art");
  assert.ok(!civicBrowse.resources.includes("/build_ncm/buildings/civic/compact-village-fire-brigade-station.json"), "category browsing must not load the fire-brigade-station JSON");
  assert.ok(!civicBrowse.resources.includes("/build_ncm/concepts/civic/compact-village-fire-brigade-station.webp"), "category browsing must not load the fire-brigade-station concept art");
  await clickBuilding(client, "civic-town-hall");
  ...
  await clickBuilding(client, "covered-village-bread-oven");
  ...
  await clickBuilding(client, "covered-village-notice-board");
  ...
  await clickBuilding(client, "stone-village-sundial");
  ...
  await clickBuilding(client, "compact-village-infirmary");
  ...
  await clickBuilding(client, "compact-village-schoolhouse");
  ...
  await clickBuilding(client, "compact-village-chapel");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-chapel' && document.querySelector('#modelSize').textContent === '23 × 29 × 30' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const chapel = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(chapel.activeCategory, "civic");
  assert.match(chapel.title, /Compact Village Chapel/);
  assert.equal(chapel.modelSize, "23 × 29 × 30");
  assert.match(chapel.payload, /^NCM3:/);
  assert.equal(chapel.voxelCount, 2291);
  for (const id of [55, 56, 57, 58, 60, 68, 69, 70, 96]) assert.match(chapel.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(chapel.uncovered, false);
  assert.equal(chapel.glazingDisabled, true);
  assert.equal(chapel.glazingLabel, "Openings: Not applicable");
  assert.equal(chapel.disabledStyles, 6);
  assert.equal(chapel.disabledRoofs, 6);
  assert.equal(chapel.conceptHidden, false);
  assert.equal(chapel.conceptLoading, "eager");
  assert.match(chapel.conceptAlt, /Compact Village Chapel concept reference/);
  assert.equal(chapel.conceptFit, "contain");
  assert.equal(chapel.selectedInUrl, "compact-village-chapel");
  assert.ok(chapel.resources.includes("/build_ncm/buildings/civic/compact-village-chapel.json"));
  assert.ok(chapel.resources.includes("/build_ncm/concepts/civic/compact-village-chapel.webp"));
  assert.ok(!chapel.resources.some((path) => path.endsWith("compact-village-chapel-blueprint.js")));
  assert.ok(!chapel.resources.includes("/build_ncm/buildings/civic/compact-village-bathhouse.json"), "selecting existing civic buildings must not load the bathhouse JSON");
  assert.ok(!chapel.resources.includes("/build_ncm/concepts/civic/compact-village-bathhouse.webp"), "selecting existing civic buildings must not load the bathhouse concept art");

  await clickBuilding(client, "compact-village-bathhouse");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-bathhouse' && document.querySelector('#modelSize').textContent === '31 × 26 × 29' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const bathhouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(bathhouse.activeCategory, "civic");
  assert.match(bathhouse.title, /Compact Village Bathhouse/);
  assert.equal(bathhouse.modelSize, "31 × 26 × 29");
  assert.match(bathhouse.payload, /^NCM3:/);
  assert.equal(bathhouse.voxelCount, 2550);
  for (const id of [55, 56, 57, 58, 62, 68, 69, 70, 74, 75, 96]) assert.match(bathhouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(bathhouse.uncovered, false);
  assert.equal(bathhouse.glazingDisabled, true);
  assert.equal(bathhouse.glazingLabel, "Openings: Not applicable");
  assert.equal(bathhouse.disabledStyles, 6);
  assert.equal(bathhouse.disabledRoofs, 6);
  assert.equal(bathhouse.conceptHidden, false);
  assert.equal(bathhouse.conceptLoading, "eager");
  assert.match(bathhouse.conceptAlt, /Compact Village Bathhouse concept reference/);
  assert.equal(bathhouse.conceptFit, "contain");
  assert.equal(bathhouse.selectedInUrl, "compact-village-bathhouse");
  assert.ok(bathhouse.resources.includes("/build_ncm/buildings/civic/compact-village-bathhouse.json"));
  assert.ok(bathhouse.resources.includes("/build_ncm/concepts/civic/compact-village-bathhouse.webp"));
  assert.ok(!bathhouse.resources.some((path) => path.endsWith("compact-village-bathhouse-blueprint.js")));

  await clickBuilding(client, "compact-village-fire-brigade-station");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-fire-brigade-station' && document.querySelector('#modelSize').textContent === '33 × 18 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const fireBrigadeStation = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(fireBrigadeStation.activeCategory, "civic");
  assert.match(fireBrigadeStation.title, /Compact Village Fire Brigade Station/);
  assert.equal(fireBrigadeStation.modelSize, "33 × 18 × 25");
  assert.match(fireBrigadeStation.payload, /^NCM3:/);
  assert.equal(fireBrigadeStation.voxelCount, 2432);
  for (const id of [55, 56, 57, 58, 65, 68, 69, 70, 74, 96]) assert.match(fireBrigadeStation.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(fireBrigadeStation.uncovered, false);
  assert.equal(fireBrigadeStation.glazingDisabled, true);
  assert.equal(fireBrigadeStation.glazingLabel, "Openings: Not applicable");
  assert.equal(fireBrigadeStation.disabledStyles, 6);
  assert.equal(fireBrigadeStation.disabledRoofs, 6);
  assert.equal(fireBrigadeStation.conceptHidden, false);
  assert.equal(fireBrigadeStation.conceptLoading, "eager");
  assert.match(fireBrigadeStation.conceptAlt, /Compact Village Fire Brigade Station concept reference/);
  assert.equal(fireBrigadeStation.conceptFit, "contain");
  assert.equal(fireBrigadeStation.selectedInUrl, "compact-village-fire-brigade-station");
  assert.ok(fireBrigadeStation.resources.includes("/build_ncm/buildings/civic/compact-village-fire-brigade-station.json"));
  assert.ok(fireBrigadeStation.resources.includes("/build_ncm/concepts/civic/compact-village-fire-brigade-station.webp"));
  assert.ok(!fireBrigadeStation.resources.some((path) => path.endsWith("compact-village-fire-brigade-station-blueprint.js")));
  ...
  assert.ok(!utilityBrowse.resources.includes("/build_ncm/buildings/utility/compact-village-laundry-house.json"), "category browsing must not load the laundry-house JSON");
  ...
  await clickBuilding(client, "village-twin-lantern");
  ...
  await clickBuilding(client, "covered-village-firewood-rack");
  ...
  assert.ok(!firewoodRack.resources.includes("/build_ncm/buildings/utility/compact-village-laundry-house.json"), "selecting existing utility buildings must not load the laundry-house JSON");
  assert.ok(!firewoodRack.resources.includes("/build_ncm/concepts/utility/compact-village-laundry-house.webp"), "selecting existing utility buildings must not load the laundry-house concept art");

  await clickBuilding(client, "compact-village-laundry-house");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-laundry-house' && document.querySelector('#modelSize').textContent === '29 × 23 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const laundryHouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(laundryHouse.activeCategory, "utility");
  assert.match(laundryHouse.title, /Compact Village Laundry House/);
  assert.equal(laundryHouse.modelSize, "29 × 23 × 25");
  assert.match(laundryHouse.payload, /^NCM3:/);
  assert.equal(laundryHouse.voxelCount, 1974);
  for (const id of [55, 56, 57, 58, 68, 69, 70, 74, 75, 99]) assert.match(laundryHouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(laundryHouse.uncovered, false);
  assert.equal(laundryHouse.glazingDisabled, true);
  assert.equal(laundryHouse.glazingLabel, "Openings: Not applicable");
  assert.equal(laundryHouse.disabledStyles, 6);
  assert.equal(laundryHouse.disabledRoofs, 6);
  assert.equal(laundryHouse.conceptHidden, false);
  assert.equal(laundryHouse.conceptLoading, "eager");
  assert.match(laundryHouse.conceptAlt, /Compact Village Laundry House concept reference/);
  assert.equal(laundryHouse.conceptFit, "contain");
  assert.equal(laundryHouse.selectedInUrl, "compact-village-laundry-house");
  assert.ok(laundryHouse.resources.includes("/build_ncm/buildings/utility/compact-village-laundry-house.json"));
  assert.ok(laundryHouse.resources.includes("/build_ncm/concepts/utility/compact-village-laundry-house.webp"));
  assert.ok(!laundryHouse.resources.some((path) => path.endsWith("compact-village-laundry-house-blueprint.js")));

  await clickBuilding(client, "compact-village-public-privy");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-public-privy' && document.querySelector('#modelSize').textContent === '27 × 18 × 23' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const publicPrivy = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(publicPrivy.activeCategory, "utility");
  assert.match(publicPrivy.title, /Compact Village Public Privy/);
  assert.equal(publicPrivy.modelSize, "27 × 18 × 23");
  assert.match(publicPrivy.payload, /^NCM3:/);
  assert.equal(publicPrivy.voxelCount, 2324);
  for (const id of [57, 58, 65, 68, 69, 70, 74, 96]) assert.match(publicPrivy.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(publicPrivy.uncovered, false);
  assert.equal(publicPrivy.glazingDisabled, true);
  assert.equal(publicPrivy.glazingLabel, "Openings: Not applicable");
  assert.equal(publicPrivy.disabledStyles, 6);
  assert.equal(publicPrivy.disabledRoofs, 6);
  assert.equal(publicPrivy.conceptHidden, false);
  assert.equal(publicPrivy.conceptLoading, "eager");
  assert.match(publicPrivy.conceptAlt, /Compact Village Public Privy concept reference/);
  assert.equal(publicPrivy.conceptFit, "contain");
  assert.equal(publicPrivy.selectedInUrl, "compact-village-public-privy");
  ...
  assert.equal(wayfindingBrowse.cardCount, 5);
  ...
  assert.ok(!wayfindingBrowse.resources.includes("/build_ncm/buildings/wayfinding/compact-village-visitor-information-kiosk.json"), "category browsing must not load the visitor-information-kiosk JSON");
  assert.ok(!wayfindingBrowse.resources.includes("/build_ncm/concepts/wayfinding/compact-village-visitor-information-kiosk.webp"), "category browsing must not load the visitor-information-kiosk concept art");
  ...
  await clickBuilding(client, "compact-village-visitor-information-kiosk");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-visitor-information-kiosk' && document.querySelector('#modelSize').textContent === '29 × 16 × 20' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const visitorInformationKiosk = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(visitorInformationKiosk.activeCategory, "wayfinding");
  assert.match(visitorInformationKiosk.title, /Compact Village Visitor Information Kiosk/);
  assert.equal(visitorInformationKiosk.modelSize, "29 × 16 × 20");
  assert.match(visitorInformationKiosk.payload, /^NCM3:/);
  assert.equal(visitorInformationKiosk.voxelCount, 1761);
  for (const id of [55, 56, 57, 58, 68, 69, 70, 74, 96]) assert.match(visitorInformationKiosk.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(visitorInformationKiosk.uncovered, false);
  assert.equal(visitorInformationKiosk.glazingDisabled, true);
  assert.equal(visitorInformationKiosk.glazingLabel, "Openings: Not applicable");
  assert.equal(visitorInformationKiosk.disabledStyles, 6);
  assert.equal(visitorInformationKiosk.disabledRoofs, 6);
  assert.equal(visitorInformationKiosk.conceptHidden, false);
  assert.equal(visitorInformationKiosk.conceptLoading, "eager");
  assert.match(visitorInformationKiosk.conceptAlt, /Compact Village Visitor Information Kiosk concept reference/);
  assert.equal(visitorInformationKiosk.conceptFit, "contain");
  assert.equal(visitorInformationKiosk.selectedInUrl, "compact-village-visitor-information-kiosk");
  assert.ok(visitorInformationKiosk.resources.includes("/build_ncm/buildings/wayfinding/compact-village-visitor-information-kiosk.json"));
  assert.ok(visitorInformationKiosk.resources.includes("/build_ncm/concepts/wayfinding/compact-village-visitor-information-kiosk.webp"));
  ...
  assert.ok(!commerceBrowse.resources.includes("/build_ncm/buildings/commerce/compact-village-tavern.json"), "category browsing must not load the tavern JSON");
  ...
  await clickBuilding(client, "compact-village-general-store");
  ...
  await clickBuilding(client, "compact-village-bakery");
  ...
  await clickBuilding(client, "compact-village-tavern");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-tavern' && document.querySelector('#modelSize').textContent === '23 × 18 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const tavern = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(tavern.activeCategory, "commerce");
  assert.match(tavern.title, /Compact Village Tavern/);
  assert.equal(tavern.modelSize, "23 × 18 × 25");
  assert.match(tavern.payload, /^NCM3:/);
  assert.equal(tavern.voxelCount, 2184);
  for (const id of [55, 56, 57, 58, 62, 66, 68, 69, 70, 96]) assert.match(tavern.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(tavern.uncovered, false);
  assert.equal(tavern.glazingDisabled, true);
  assert.equal(tavern.glazingLabel, "Openings: Not applicable");
  assert.equal(tavern.disabledStyles, 6);
  assert.equal(tavern.disabledRoofs, 6);
  assert.equal(tavern.conceptHidden, false);
  assert.match(tavern.conceptAlt, /Compact Village Tavern concept reference/);
  assert.equal(tavern.conceptFit, "contain");
  assert.equal(tavern.selectedInUrl, "compact-village-tavern");
  assert.ok(tavern.resources.includes("/build_ncm/buildings/commerce/compact-village-tavern.json"));
  ...
  assert.ok(!tavern.resources.includes("/build_ncm/buildings/commerce/compact-village-butcher-shop.json"), "selecting existing commerce buildings must not load the butcher-shop JSON");
  assert.ok(!tavern.resources.includes("/build_ncm/concepts/commerce/compact-village-butcher-shop.webp"), "selecting existing commerce buildings must not load the butcher-shop concept art");

  await clickBuilding(client, "compact-village-butcher-shop");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-butcher-shop' && document.querySelector('#modelSize').textContent === '25 × 21 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const butcherShop = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(butcherShop.activeCategory, "commerce");
  assert.match(butcherShop.title, /Compact Village Butcher Shop/);
  assert.equal(butcherShop.modelSize, "25 × 21 × 25");
  assert.match(butcherShop.payload, /^NCM3:/);
  assert.equal(butcherShop.voxelCount, 1821);
  for (const id of [55, 56, 57, 58, 62, 68, 69, 70, 74, 96]) assert.match(butcherShop.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(butcherShop.uncovered, false);
  assert.equal(butcherShop.glazingDisabled, true);
  assert.equal(butcherShop.glazingLabel, "Openings: Not applicable");
  assert.equal(butcherShop.disabledStyles, 6);
  assert.equal(butcherShop.disabledRoofs, 6);
  assert.equal(butcherShop.conceptHidden, false);
  assert.equal(butcherShop.conceptLoading, "eager");
  assert.match(butcherShop.conceptAlt, /Compact Village Butcher Shop concept reference/);
  assert.equal(butcherShop.conceptFit, "contain");
  assert.equal(butcherShop.selectedInUrl, "compact-village-butcher-shop");
  ...
  assert.equal(transportBrowse.cardCount, 5);
  ...
  assert.ok(!transportBrowse.resources.includes("/build_ncm/buildings/transport/compact-village-carriage-house.json"), "category browsing must not load the carriage-house JSON");
  assert.ok(!transportBrowse.resources.includes("/build_ncm/concepts/transport/compact-village-carriage-house.webp"), "category browsing must not load the carriage-house concept art");
  assert.ok(!transportBrowse.resources.includes("/build_ncm/buildings/transport/compact-village-courier-post.json"), "category browsing must not load the courier-post JSON");
  assert.ok(!transportBrowse.resources.includes("/build_ncm/concepts/transport/compact-village-courier-post.webp"), "category browsing must not load the courier-post concept art");
  assert.ok(!transportBrowse.resources.includes("/build_ncm/buildings/transport/covered-village-ferry-landing.json"), "category browsing must not load the ferry-landing JSON");
  assert.ok(!transportBrowse.resources.includes("/build_ncm/concepts/transport/covered-village-ferry-landing.webp"), "category browsing must not load the ferry-landing concept art");
  await clickBuilding(client, "stone-timber-footbridge");
  ...
  await clickBuilding(client, "two-wheel-village-handcart");
  ...
  assert.ok(!handcart.resources.includes("/build_ncm/buildings/transport/compact-village-carriage-house.json"), "selecting existing transport buildings must not load the carriage-house JSON");
  assert.ok(!handcart.resources.includes("/build_ncm/concepts/transport/compact-village-carriage-house.webp"), "selecting existing transport buildings must not load the carriage-house concept art");

  await clickBuilding(client, "compact-village-carriage-house");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-carriage-house' && document.querySelector('#modelSize').textContent === '33 × 25 × 31' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const carriageHouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(carriageHouse.activeCategory, "transport");
  assert.match(carriageHouse.title, /Compact Village Carriage House/);
  assert.equal(carriageHouse.modelSize, "33 × 25 × 31");
  assert.match(carriageHouse.payload, /^NCM3:/);
  assert.equal(carriageHouse.voxelCount, 2639);
  for (const id of [55, 56, 57, 58, 68, 69, 70, 99]) assert.match(carriageHouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(carriageHouse.uncovered, false);
  assert.equal(carriageHouse.glazingDisabled, true);
  assert.equal(carriageHouse.glazingLabel, "Openings: Not applicable");
  assert.equal(carriageHouse.disabledStyles, 6);
  assert.equal(carriageHouse.disabledRoofs, 6);
  assert.equal(carriageHouse.conceptHidden, false);
  assert.equal(carriageHouse.conceptLoading, "eager");
  assert.match(carriageHouse.conceptAlt, /Compact Village Carriage House concept reference/);
  assert.equal(carriageHouse.conceptFit, "contain");
  assert.equal(carriageHouse.selectedInUrl, "compact-village-carriage-house");
  assert.ok(carriageHouse.resources.includes("/build_ncm/buildings/transport/compact-village-carriage-house.json"));
  assert.ok(carriageHouse.resources.includes("/build_ncm/concepts/transport/compact-village-carriage-house.webp"));
  assert.ok(!carriageHouse.resources.some((path) => path.endsWith("compact-village-carriage-house-blueprint.js")));
  assert.ok(!carriageHouse.resources.includes("/build_ncm/buildings/transport/compact-village-courier-post.json"), "selecting existing transport buildings must not load the courier-post JSON");
  assert.ok(!carriageHouse.resources.includes("/build_ncm/concepts/transport/compact-village-courier-post.webp"), "selecting existing transport buildings must not load the courier-post concept art");

  await clickBuilding(client, "compact-village-courier-post");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-courier-post' && document.querySelector('#modelSize').textContent === '31 × 23 × 27' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const courierPost = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(courierPost.activeCategory, "transport");
  assert.match(courierPost.title, /Compact Village Courier Post/);
  assert.equal(courierPost.modelSize, "31 × 23 × 27");
  assert.match(courierPost.payload, /^NCM3:/);
  assert.equal(courierPost.voxelCount, 1978);
  for (const id of [55, 56, 57, 58, 62, 68, 69, 70, 99]) assert.match(courierPost.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(courierPost.uncovered, false);
  assert.equal(courierPost.glazingDisabled, true);
  assert.equal(courierPost.glazingLabel, "Openings: Not applicable");
  assert.equal(courierPost.disabledStyles, 6);
  assert.equal(courierPost.disabledRoofs, 6);
  assert.equal(courierPost.conceptHidden, false);
  assert.equal(courierPost.conceptLoading, "eager");
  assert.match(courierPost.conceptAlt, /Compact Village Courier Post concept reference/);
  assert.equal(courierPost.conceptFit, "contain");
  assert.equal(courierPost.selectedInUrl, "compact-village-courier-post");
  assert.ok(courierPost.resources.includes("/build_ncm/buildings/transport/compact-village-courier-post.json"));
  assert.ok(courierPost.resources.includes("/build_ncm/concepts/transport/compact-village-courier-post.webp"));
  assert.ok(!courierPost.resources.some((path) => path.endsWith("compact-village-courier-post-blueprint.js")));

  await clickBuilding(client, "covered-village-ferry-landing");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'covered-village-ferry-landing' && document.querySelector('#modelSize').textContent === '19 × 21 × 29' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const ferryLanding = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(ferryLanding.activeCategory, "transport");
  assert.match(ferryLanding.title, /Covered Village Ferry Landing/);
  assert.equal(ferryLanding.modelSize, "19 × 21 × 29");
  assert.match(ferryLanding.payload, /^NCM3:/);
  assert.equal(ferryLanding.voxelCount, 1066);
  for (const id of [55, 56, 57, 68, 69, 99]) assert.match(ferryLanding.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(ferryLanding.uncovered, false);
  assert.equal(ferryLanding.glazingDisabled, true);
  assert.equal(ferryLanding.glazingLabel, "Openings: Not applicable");
  assert.equal(ferryLanding.disabledStyles, 6);
  assert.equal(ferryLanding.disabledRoofs, 6);
  assert.equal(ferryLanding.conceptHidden, false);
  assert.equal(ferryLanding.conceptLoading, "eager");
  assert.match(ferryLanding.conceptAlt, /Covered Village Ferry Landing concept reference/);
  assert.equal(ferryLanding.conceptFit, "contain");
  assert.equal(ferryLanding.selectedInUrl, "covered-village-ferry-landing");
  assert.ok(ferryLanding.resources.includes("/build_ncm/buildings/transport/covered-village-ferry-landing.json"));
  assert.ok(ferryLanding.resources.includes("/build_ncm/concepts/transport/covered-village-ferry-landing.webp"));
  ...
  assert.ok(!headframe.resources.includes("/build_ncm/buildings/mining/compact-village-lamp-room.json"), "selecting the headframe must not load the lamp-room JSON");
  assert.ok(!headframe.resources.includes("/build_ncm/concepts/mining/compact-village-lamp-room.webp"), "selecting the headframe must not load the lamp-room concept art");

  await clickBuilding(client, "covered-village-ore-sorting-shed");
  ...
  assert.ok(!oreSortingShed.resources.includes("/build_ncm/buildings/mining/compact-village-lamp-room.json"), "selecting the ore-sorting shed must not load the lamp-room JSON");
  assert.ok(!oreSortingShed.resources.includes("/build_ncm/concepts/mining/compact-village-lamp-room.webp"), "selecting the ore-sorting shed must not load the lamp-room concept art");
  ...
  assert.ok(!assayHouse.resources.includes("/build_ncm/buildings/mining/compact-village-lamp-room.json"), "selecting the assay house must not load the lamp-room JSON");
  assert.ok(!assayHouse.resources.includes("/build_ncm/concepts/mining/compact-village-lamp-room.webp"), "selecting the assay house must not load the lamp-room concept art");

  await clickBuilding(client, "compact-village-lamp-room");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-lamp-room' && document.querySelector('#modelSize').textContent === '25 × 20 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const lampRoom = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(lampRoom.activeCategory, "mining");
  assert.match(lampRoom.title, /Compact Village Lamp Room/);
  assert.equal(lampRoom.modelSize, "25 × 20 × 25");
  assert.match(lampRoom.payload, /^NCM3:/);
  assert.equal(lampRoom.voxelCount, 1858);
  for (const id of [56, 57, 58, 64, 65, 69, 70, 99]) assert.match(lampRoom.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(lampRoom.uncovered, false);
  assert.equal(lampRoom.glazingDisabled, true);
  assert.equal(lampRoom.glazingLabel, "Openings: Not applicable");
  assert.equal(lampRoom.disabledStyles, 6);
  assert.equal(lampRoom.disabledRoofs, 6);
  assert.equal(lampRoom.conceptHidden, false);
  assert.equal(lampRoom.conceptLoading, "eager");
  assert.match(lampRoom.conceptAlt, /Compact Village Lamp Room concept reference/);
  assert.equal(lampRoom.conceptFit, "contain");
  assert.equal(lampRoom.selectedInUrl, "compact-village-lamp-room");
  ...
  await waitFor(() => evaluate(client, "document.querySelector('[data-building=glass-timber-greenhouse]') && document.querySelector('[data-building=compact-village-watermill]') && document.querySelector('[data-building=compact-village-cheese-house]') && document.querySelector('[data-building=compact-village-poultry-house]') && document.querySelector('[data-building=compact-village-piggery]') && document.querySelector('[data-building=compact-village-cider-press-house]')"));
  ...
  assert.equal(agricultureBrowse.cardCount, 10);
  ...
  assert.ok(!agricultureBrowse.resources.includes("/build_ncm/buildings/agriculture/compact-village-granary.json"), "category browsing must not load the granary JSON");
  ...
  assert.ok(!agricultureBrowse.resources.includes("/build_ncm/buildings/agriculture/compact-village-poultry-house.json"), "category browsing must not load the poultry-house JSON");
  assert.ok(!agricultureBrowse.resources.includes("/build_ncm/concepts/agriculture/compact-village-poultry-house.webp"), "category browsing must not load the poultry-house concept art");
  assert.ok(!agricultureBrowse.resources.includes("/build_ncm/buildings/agriculture/compact-village-piggery.json"), "category browsing must not load the piggery JSON");
  assert.ok(!agricultureBrowse.resources.includes("/build_ncm/concepts/agriculture/compact-village-piggery.webp"), "category browsing must not load the piggery concept art");
  assert.ok(!agricultureBrowse.resources.includes("/build_ncm/buildings/agriculture/compact-village-cider-press-house.json"), "category browsing must not load the cider-press-house JSON");
  assert.ok(!agricultureBrowse.resources.includes("/build_ncm/concepts/agriculture/compact-village-cider-press-house.webp"), "category browsing must not load the cider-press-house concept art");
  ...
  await clickBuilding(client, "stone-timber-tower-windmill");
  ...
  await clickBuilding(client, "covered-village-apiary");
  ...
  await clickBuilding(client, "compact-village-stable-barn");
  ...
  assert.ok(!stableBarn.resources.includes("/build_ncm/buildings/agriculture/compact-village-granary.json"), "selecting the stable barn must not load the granary JSON");
  assert.ok(!stableBarn.resources.includes("/build_ncm/concepts/agriculture/compact-village-granary.webp"), "selecting the stable barn must not load the granary concept art");
  assert.ok(!stableBarn.resources.some((path) => path.endsWith("compact-village-stable-barn-blueprint.js")));

  await clickBuilding(client, "compact-village-granary");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-granary' && document.querySelector('#modelSize').textContent === '23 × 16 × 21' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const granary = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(granary.activeCategory, "agriculture");
  assert.match(granary.title, /Compact Village Granary/);
  assert.equal(granary.modelSize, "23 × 16 × 21");
  assert.match(granary.payload, /^NCM3:/);
  assert.equal(granary.voxelCount, 2094);
  for (const id of [55, 56, 57, 58, 68, 70, 72, 96]) assert.match(granary.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(granary.uncovered, false);
  assert.equal(granary.glazingDisabled, true);
  assert.equal(granary.glazingLabel, "Openings: Not applicable");
  assert.equal(granary.disabledStyles, 6);
  assert.equal(granary.disabledRoofs, 6);
  assert.equal(granary.conceptHidden, false);
  assert.match(granary.conceptAlt, /Compact Village Granary concept reference/);
  assert.equal(granary.conceptFit, "contain");
  assert.equal(granary.selectedInUrl, "compact-village-granary");
  assert.ok(granary.resources.includes("/build_ncm/buildings/agriculture/compact-village-granary.json"));
  ...
  assert.ok(!granary.resources.includes("/build_ncm/buildings/agriculture/compact-village-watermill.json"), "selecting the granary must not load the watermill JSON");
  assert.ok(!granary.resources.includes("/build_ncm/concepts/agriculture/compact-village-watermill.webp"), "selecting the granary must not load the watermill concept art");

  await clickBuilding(client, "compact-village-watermill");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-watermill' && document.querySelector('#modelSize').textContent === '27 × 30 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const watermill = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(watermill.activeCategory, "agriculture");
  assert.match(watermill.title, /Compact Village Watermill/);
  assert.equal(watermill.modelSize, "27 × 30 × 25");
  assert.match(watermill.payload, /^NCM3:/);
  assert.equal(watermill.voxelCount, 2864);
  for (const id of [55, 56, 57, 58, 65, 68, 69, 70, 96]) assert.match(watermill.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(watermill.uncovered, false);
  assert.equal(watermill.glazingDisabled, true);
  assert.equal(watermill.glazingLabel, "Openings: Not applicable");
  assert.equal(watermill.disabledStyles, 6);
  assert.equal(watermill.disabledRoofs, 6);
  assert.equal(watermill.conceptHidden, false);
  assert.equal(watermill.conceptLoading, "eager");
  assert.match(watermill.conceptAlt, /Compact Village Watermill concept reference/);
  assert.equal(watermill.conceptFit, "contain");
  assert.equal(watermill.selectedInUrl, "compact-village-watermill");
  ...
  await clickBuilding(client, "compact-village-poultry-house");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-poultry-house' && document.querySelector('#modelSize').textContent === '31 × 18 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const poultryHouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(poultryHouse.activeCategory, "agriculture");
  assert.match(poultryHouse.title, /Compact Village Poultry House/);
  assert.equal(poultryHouse.modelSize, "31 × 18 × 25");
  assert.match(poultryHouse.payload, /^NCM3:/);
  assert.equal(poultryHouse.voxelCount, 2347);
  for (const id of [55, 56, 57, 58, 68, 69, 70, 72, 96]) assert.match(poultryHouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(poultryHouse.uncovered, false);
  assert.equal(poultryHouse.glazingDisabled, true);
  assert.equal(poultryHouse.glazingLabel, "Openings: Not applicable");
  assert.equal(poultryHouse.disabledStyles, 6);
  assert.equal(poultryHouse.disabledRoofs, 6);
  assert.equal(poultryHouse.conceptHidden, false);
  assert.equal(poultryHouse.conceptLoading, "eager");
  assert.match(poultryHouse.conceptAlt, /Compact Village Poultry House concept reference/);
  assert.equal(poultryHouse.conceptFit, "contain");
  assert.equal(poultryHouse.selectedInUrl, "compact-village-poultry-house");
  assert.ok(poultryHouse.resources.includes("/build_ncm/buildings/agriculture/compact-village-poultry-house.json"));
  assert.ok(poultryHouse.resources.includes("/build_ncm/concepts/agriculture/compact-village-poultry-house.webp"));
  assert.ok(!poultryHouse.resources.some((path) => path.endsWith("compact-village-poultry-house-blueprint.js")));

  await clickBuilding(client, "compact-village-piggery");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-piggery' && document.querySelector('#modelSize').textContent === '29 × 19 × 25' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const piggery = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(piggery.activeCategory, "agriculture");
  assert.match(piggery.title, /Compact Village Piggery/);
  assert.equal(piggery.modelSize, "29 × 19 × 25");
  assert.match(piggery.payload, /^NCM3:/);
  assert.equal(piggery.voxelCount, 2408);
  for (const id of [55, 56, 57, 58, 68, 69, 70, 96]) assert.match(piggery.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(piggery.uncovered, false);
  assert.equal(piggery.glazingDisabled, true);
  assert.equal(piggery.glazingLabel, "Openings: Not applicable");
  assert.equal(piggery.disabledStyles, 6);
  assert.equal(piggery.disabledRoofs, 6);
  assert.equal(piggery.conceptHidden, false);
  assert.equal(piggery.conceptLoading, "eager");
  assert.match(piggery.conceptAlt, /Compact Village Piggery concept reference/);
  assert.equal(piggery.conceptFit, "contain");
  assert.equal(piggery.selectedInUrl, "compact-village-piggery");
  assert.ok(piggery.resources.includes("/build_ncm/buildings/agriculture/compact-village-piggery.json"));
  assert.ok(piggery.resources.includes("/build_ncm/concepts/agriculture/compact-village-piggery.webp"));
  assert.ok(!piggery.resources.some((path) => path.endsWith("compact-village-piggery-blueprint.js")));
  assert.ok(!piggery.resources.includes("/build_ncm/buildings/agriculture/compact-village-cider-press-house.json"), "selecting existing agriculture buildings must not load the cider-press-house JSON");
  assert.ok(!piggery.resources.includes("/build_ncm/concepts/agriculture/compact-village-cider-press-house.webp"), "selecting existing agriculture buildings must not load the cider-press-house concept art");

  await clickBuilding(client, "compact-village-cider-press-house");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'compact-village-cider-press-house' && document.querySelector('#modelSize').textContent === '29 × 20 × 27' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const ciderPressHouse = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(ciderPressHouse.activeCategory, "agriculture");
  assert.match(ciderPressHouse.title, /Compact Village Cider Press House/);
  assert.equal(ciderPressHouse.modelSize, "29 × 20 × 27");
  assert.match(ciderPressHouse.payload, /^NCM3:/);
  assert.equal(ciderPressHouse.voxelCount, 2847);
  for (const id of [55, 56, 57, 58, 65, 68, 69, 70, 96]) assert.match(ciderPressHouse.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(ciderPressHouse.uncovered, false);
  assert.equal(ciderPressHouse.glazingDisabled, true);
  assert.equal(ciderPressHouse.glazingLabel, "Openings: Not applicable");
  assert.equal(ciderPressHouse.disabledStyles, 6);
  assert.equal(ciderPressHouse.disabledRoofs, 6);
  assert.equal(ciderPressHouse.conceptHidden, false);
  assert.equal(ciderPressHouse.conceptLoading, "eager");
  assert.match(ciderPressHouse.conceptAlt, /Compact Village Cider Press House concept reference/);
  assert.equal(ciderPressHouse.conceptFit, "contain");
  assert.equal(ciderPressHouse.selectedInUrl, "compact-village-cider-press-house");
  assert.ok(ciderPressHouse.resources.includes("/build_ncm/buildings/agriculture/compact-village-cider-press-house.json"));
  assert.ok(ciderPressHouse.resources.includes("/build_ncm/concepts/agriculture/compact-village-cider-press-house.webp"));
  assert.ok(!ciderPressHouse.resources.some((path) => path.endsWith("compact-village-cider-press-house-blueprint.js")));

  await evaluate(client, "document.querySelector('[data-building-category=construction]').click()");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building-category].active')?.dataset.buildingCategory === 'construction' && document.querySelectorAll('[data-building]').length === 5 && document.querySelector('[data-building=timber-building-scaffold]') && document.querySelector('[data-building=compact-village-stonemason-workshop]') && document.querySelector('[data-building=compact-village-carpenter-workshop]') && document.querySelector('[data-building=compact-village-building-supply-depot]') && document.querySelector('[data-building=timber-village-building-gantry]')"));
  ...
  assert.match(constructionBrowse.previewTitle, /Compact Village Cheese House|Compact Village Poultry House|Compact Village Piggery|Compact Village Cider Press House/);
  ...
  assert.ok(!constructionBrowse.resources.includes("/build_ncm/buildings/construction/timber-village-building-gantry.json"), "browsing construction must not load the building-gantry JSON");
  assert.ok(!constructionBrowse.resources.includes("/build_ncm/concepts/construction/timber-village-building-gantry.webp"), "browsing construction must not load the building-gantry concept art");
  ...
  await clickBuilding(client, "timber-village-building-gantry");
  await waitFor(() => evaluate(client, "document.querySelector('[data-building].active')?.dataset.building === 'timber-village-building-gantry' && document.querySelector('#modelSize').textContent === '31 × 26 × 23' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  const buildingGantry = await evaluate(client, `({
    activeCategory: document.querySelector('[data-building-category].active')?.dataset.buildingCategory,
    title: document.querySelector('#buildingTitle').textContent,
    modelSize: document.querySelector('#modelSize').textContent,
    payload: document.querySelector('#codeOutput').value,
  ...
  assert.equal(buildingGantry.activeCategory, "construction");
  assert.match(buildingGantry.title, /Timber Village Building Gantry/);
  assert.equal(buildingGantry.modelSize, "31 × 26 × 23");
  assert.match(buildingGantry.payload, /^NCM3:/);
  assert.equal(buildingGantry.voxelCount, 1709);
  for (const id of [56, 57, 64, 65, 68, 69]) assert.match(buildingGantry.usedMaterials, new RegExp(`MAT_${String(id).padStart(3, '0')}`));
  assert.equal(buildingGantry.uncovered, false);
  assert.equal(buildingGantry.glazingDisabled, true);
  assert.equal(buildingGantry.glazingLabel, "Openings: Not applicable");
  assert.equal(buildingGantry.disabledStyles, 6);
  assert.equal(buildingGantry.disabledRoofs, 6);
  assert.equal(buildingGantry.conceptHidden, false);
  assert.equal(buildingGantry.conceptLoading, "eager");
  assert.match(buildingGantry.conceptAlt, /Timber Village Building Gantry concept reference/);
  assert.equal(buildingGantry.conceptFit, "contain");
  assert.equal(buildingGantry.selectedInUrl, "timber-village-building-gantry");
  assert.ok(buildingGantry.resources.includes("/build_ncm/buildings/construction/timber-village-building-gantry.json"));
  assert.ok(buildingGantry.resources.includes("/build_ncm/concepts/construction/timber-village-building-gantry.webp"));
  assert.ok(!buildingGantry.resources.some((path) => path.endsWith("timber-village-building-gantry-blueprint.js")));
  ...
  await waitFor(() => evaluate(client, "document.readyState === 'complete' && document.querySelectorAll('[data-style]').length === 6 && document.querySelectorAll('[data-building-category]').length === 12 && document.querySelectorAll('[data-building]').length === 5"));
  await evaluate(client, "document.querySelector('[data-material-filter=all]').click()");
  await waitFor(() => evaluate(client, "document.querySelectorAll('#buildingMaterialCatalog .model-material-card').length === 33"));
  ...
  const boathouseDirectUrl = new URL(url);
  boathouseDirectUrl.searchParams.set("building", "compact-village-boathouse");
  await client.send("Page.navigate", { url: boathouseDirectUrl.href });
  await waitFor(() => evaluate(client, "document.readyState === 'complete' && document.querySelector('[data-building-category].active')?.dataset.buildingCategory === 'coastal' && document.querySelector('[data-building].active')?.dataset.building === 'compact-village-boathouse' && document.querySelector('#conceptImage').complete && document.querySelector('#conceptImage').naturalWidth > 0"));
  ...
  assert.equal(directSelection.activeBuilding, "compact-village-boathouse");
  assert.match(directSelection.title, /Compact Village Boathouse/);
  assert.equal(directSelection.modelSize, "35 × 22 × 31");
  ...
  assert.equal(directSelection.conceptPath, "/build_ncm/concepts/coastal/compact-village-boathouse.webp");
  assert.ok(directSelection.resources.includes("/build_ncm/buildings/coastal/compact-village-boathouse.json"));
  assert.ok(directSelection.resources.includes("/build_ncm/concepts/coastal/compact-village-boathouse.webp"));
  assert.equal(
...
async function clickBuilding(client, key) {
  const encodedKey = JSON.stringify(key);
  await waitFor(() => evaluate(client, `(() => {
    const button = document.querySelector('[data-building=' + ${encodedKey} + ']');
    if (!button) return false;
    if (button.classList.contains('active')) return true;
    if (!button.classList.contains('pending')) button.click();
    return false;
  })()`));
}
