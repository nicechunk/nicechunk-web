...
export async function setLocale(nextLocale, { persist = true } = {}) {
  ...
  window.dispatchEvent(new CustomEvent("itemncm:localechange", { detail: { locale } }));
  return locale;
}

export function onLocaleChange(listener) {
  const handler = (event) => listener(event.detail.locale);
  window.addEventListener("itemncm:localechange", handler);
  return () => window.removeEventListener("itemncm:localechange", handler);
}
...
async function fetchLocaleMessages(requestedLocale) {
  ...
  if (!response.ok) throw new Error(`ITEM_NCM locale ${requestedLocale} failed with HTTP ${response.status}.`);
  const value = await response.json();
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new TypeError(`ITEM_NCM locale ${requestedLocale} is invalid.`);
...
function normalizeLocale(value) {
  ...
  if (["zh-tw", "zh-hk", "zh-mo"].includes(lower) || lower.startsWith("zh-hant")) return "zh-Hant";
